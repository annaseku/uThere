...
CREATE POLICY "Users can insert group members" ON public.group_members FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id OR EXISTS (SELECT 1 FROM group_members existing WHERE existing.group_id = group_members.group_id AND existing.user_id = auth.uid() AND existing.role = 'admin'));
CREATE POLICY "Admins can remove group members" ON public.group_members FOR DELETE TO authenticated USING (user_id = auth.uid() OR EXISTS (SELECT 1 FROM group_members existing WHERE existing.group_id = group_members.group_id AND existing.user_id = auth.uid() AND existing.role = 'admin'));
...
-- Add color_scheme and dark_mode columns to users table for persistence
ALTER TABLE public.users ADD COLUMN IF NOT EXISTS color_scheme text DEFAULT 'theme-blue';
